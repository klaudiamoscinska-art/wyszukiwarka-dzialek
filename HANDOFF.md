...
static/
  ...
  manifest.json
  service-worker.js          — CACHE_NAME="analiza-dzialki-v7", network-first,
...
**Aktualny numer: v=15.** Przy każdej zmianie `app.js` podbij `N` o 1 i
...
2. iOS Safari w trybie „dodane do ekranu głównego" rzadko sprawdza, czy
   ...
   Aktualny `CACHE_NAME`: `analiza-dzialki-v7`.
...
| Plany zagospodarowania | **Dwa źródła próbowane po kolei**: (1) nowy `KrajowaIntegracjaAktowPlanowaniaPrzestrzennego` (Rejestr Urbanistyczny, wystartował 1 lipca 2026, **obecnie ogólnokrajowo prawie pusty** — gminy dopiero wgrywają dane), (2) stary `KrajowaIntegracjaMiejscowychPlanowZagospodarowaniaPrzestrzennego` jako fallback | Oba używają strategii: `GetMap` (szybkie, niezawodne) jako sonda, dopiero potem `GetFeatureInfo` z limitem czasu (część serwerów gmin wisi w nieskończoność na `GetFeatureInfo`). **Historia**: usunięte jako warstwy mapy 2026-09-03, przywrócone tego samego dnia na życzenie Klaudii — teraz zbiorczy checkbox "Plany zagospodarowania" (`L.layerGroup` z obu źródeł naraz, bo to dwa różne serwery WMS, nie da się połączyć w jedno GetMap jak w GESUT) + 2 podkategorie (MPZP starszy / Rejestr Urbanistyczny) w rozwijanym `<details>`. Tabelaryczny panel (sekcja 5 w `main.py`) bez zmian przez całą tę historię. **Zdiagnozowane 2026-09-03**: Klaudia zgłosiła "checkbox jest, ale warstwa się nie rysuje" dla tej warstwy. Zweryfikowane w Playwright (podstawione odpowiedzi WMS): kod poprawnie wysyła `GetMap` z właściwym `bbox`/`layers` i renderuje to, co przyjdzie — **to nie błąd kodu**. Zapytana, czy panel tekstowy (sekcja 5, ten sam adres) też pokazuje "brak planu" — potwierdziła, że tak. Wniosek: dla tej konkretnej działki po prostu nie ma wgranego planu w żadnym z dwóch rejestrów krajowych (ten sam strukturalny problem co reszta tej tabeli — usługa działa, danych po prostu nie ma). **Jeśli to się powtórzy**: najpierw sprawdź panel tekstowy dla tego samego adresu — jeśli on też mówi "brak planu", to nie błąd renderowania mapy, tylko brak danych u źródła. Dopiero jeśli panel POKAZUJE plan, a mapa nic nie rysuje, warto zbadać hipotezę CRS: backend (`services/zoning.py` i `services/utilities.py`) celowo pyta ten serwer o `SRS=EPSG:2180` (PUWG 1992), a frontendowa warstwa kafelkowa Leaflet domyślnie pyta w `EPSG:3857` — nie sprawdzone na żywo (serwery rządowe zablokowane w sandboksie), czy `mapy.geoportal.gov.pl` poprawnie reprojektuje do 3857, czy nie — to by wymagało `proj4leaflet` żeby wymusić 2180 na pojedynczej warstwie WMS. |
