...
| Media/GESUT | KIUT, metoda pikselowa przez `GetMap` (GetFeatureInfo strukturalnie zepsute — zwraca ten sam komunikat zawsze) | Działa, ale to przybliżenie wizualne, nie dokładne atrybuty. **Od 2026-09-03 też jako warstwa mapy** ("Media / uzbrojenie terenu (GESUT)" w przełączniku obok EGiB/MPZP/Rejestru Urbanistycznego, w `static/app.js`) — ten sam serwer KIUT co panel, wszystkie 6 typów sieci (woda/kanalizacja/gaz/prąd/ciepło/telekom) w jednej warstwie WMS GetMap. Wyłączona domyślnie, jak MPZP/APP. Zweryfikowana strukturalnie (kontrolka warstw renderuje się, toggle działa bez błędów JS) w sandboksie bez dostępu do serwerów rządowych — samego renderowania kafelków KIUT nie dało się zobaczyć na żywo, sprawdź to na wdrożonej appce |
